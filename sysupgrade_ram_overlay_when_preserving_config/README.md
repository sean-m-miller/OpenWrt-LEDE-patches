...
This is also "sloppy" because the jffs2 driver does not clean up the blocks at the beginning of the rootfs_data partition that were used to preserve the config. Those blocks get cleaned up before use at some point during run time. Certainly, we should be launching the jffs2 overlay with a totally clean rootfs_data partition.
